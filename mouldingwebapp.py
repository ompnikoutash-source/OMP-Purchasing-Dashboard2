"""
Moulding Inventory Planning Web Application
This version is specifically tailored for moulding items that DO NOT have "SF" as their sales unit of measurement.
Designed for moulding buyers to manage moulding items (LF, EA, etc.)

Based on sundrieswebapp.py with JSON output for display in flooringwebapp.py Dilan tab.
"""
...
def load_moulding_list(xlsx_path: Path) -> set:
    print(f"\nLoading moulding SKU list from {xlsx_path}...")
    if not xlsx_path.exists():
        raise FileNotFoundError(f"Required file not found: {xlsx_path}")
    try:
        df = pd.read_excel(xlsx_path, sheet_name="items", header=0)
        if df.shape[0] == 0 or df.shape[1] < 1:
            raise ValueError(f"File is empty: {xlsx_path}")
        # MouldingSKUList.xlsx has "ITEM" as the first column
        sku_col = "ITEM" if "ITEM" in df.columns else df.columns[0]
        ...
        print(f"  Loaded {len(sku_list)} moulding SKUs from list")
        return sku_list
    except Exception as e:
...
def _fetch_sku_master(conn, focus_sku: str = "", cutoff_date_str: str = CUTOFF_DATE, moulding_sku_list: set = None) -> pd.DataFrame:
...
def load_trailing_12m_volume_moulding(conn, sku_list: List[str], as_of_date: Optional[pd.Timestamp] = None) -> Tuple[Dict[str, float], float]:
...
def run_inventory_planning():
    ...
    sku_list_path = output_dir / MOULDING_LIST_FILE
    moulding_sku_list = load_moulding_list(sku_list_path)
    conn = _connect()
    global AS_OF_DATE
    ...
    if single_sku:
        ...
        if focus not in active_set:
            ...
            conn.close()
            return
        moulding_sku_list = {focus}
    else:
        before_ct = len(moulding_sku_list)
        active_set, last_sale_map = filter_skus_to_active_last_365_days(conn, moulding_sku_list, days=365, as_of_date=AS_OF_DATE)
        moulding_sku_list = active_set
        after_ct = len(moulding_sku_list)
        print(f"\nActive SKU Filter: {before_ct} -> {after_ct} SKUs")
        if after_ct == 0:
            print("\nNo active SKUs found")
            conn.close()
            return
    sku_master = _fetch_sku_master(conn, FOCUS_SKU, CUTOFF_DATE, moulding_sku_list)
    if sku_master.empty:
        print("\nNo moulding SKUs to process")
        conn.close()
        return
    sku_list_for_volume = sku_master["sku"].astype(str).str.upper().tolist()
    shipped_12m_map, total_vol_12m = load_trailing_12m_volume_moulding(conn, sku_list_for_volume, as_of_date=AS_OF_DATE)
...
def run_streamlit_app():
    ...
    st.set_page_config(page_title="OMP Moulding Dashboard", layout="wide", initial_sidebar_state="expanded")
...
if __name__ == "__main__":
    if _is_streamlit_runtime():
        ...
        print("="*70)
        print("MOULDING INVENTORY PLANNING WEB APPLICATION")
        print("="*70)
